...
echo ""
echo "--- tes perintah systemd (5 detik) ---"
timeout 5 /usr/bin/flock -n "$(pwd)/database/.scheduler.lock" ./venv/bin/python scheduler.py 2>&1 || echo "(timeout/exit — lihat pesan di atas)"
echo ""
echo "=== Fix cepat ==="
