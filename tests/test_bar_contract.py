...
from core.data_provider import _rows_to_ohlc_df


class BarContractTests(unittest.TestCase):
    def test_daily_provider_bars_are_signal_eligible(self):
        frame = _rows_to_ohlc_df(
            [{"date": "2026-07-17T00:00:00Z", "open": 10, "high": 12,
              "low": 9, "close": 11, "volume": 100}]
        )
        contract = require_price_signal_eligible(frame)
        self.assertEqual(contract, INVEZGO_DAILY_CONTRACT)
    ...
    def test_attach_preserves_rows_and_records_contract(self):
        ...
        returned = attach_bar_contract(frame, INVEZGO_DAILY_CONTRACT)
        self.assertIs(returned, frame)
        self.assertEqual(len(returned), 2)
        self.assertEqual(read_bar_contract(returned).scheme, "invezgo_daily_v1")
